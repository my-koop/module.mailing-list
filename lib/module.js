...
var Module = (function (_super) {
    ...
    Module.prototype.__getMailingLists = function (connection, params, callback) {
        async.waterfall([
            function (callback) {
                ...
                connection.query("SELECT idMailingList AS id, name, description, showAtRegistration \
    ...
    Module.prototype.getConfigurations = function (params, callback) {
        this.callWithConnection(this.__getConfigurations, params, callback);
    };
    Module.prototype.__getConfigurations = function (connection, params, callback) {
        connection.query("SELECT value FROM configuration WHERE `key` = ?", ["mailingList"], function (err, res) {
            callback(err && new DatabaseError(err), (res && res.length === 1 && JSON.parse(res[0].value)) || null);
        });
    };
    Module.prototype.setConfigurations = function (params, callback) {
        this.callWithConnection(this.__setConfigurations, params, callback);
    };
    Module.prototype.__setConfigurations = function (connection, params, callback) {
        var self = this;
        ...
        async.waterfall([
            ...
            function (configs, next) {
                ...
                if (!configs) {
                    ...
                    return connection.query("INSERT INTO configuration SET `key` = ?, value = ?", ["mailingList", newConfigsString], function (err) {
                        if (err) {
                            logger.debug(err);
                        }
            ...
            function (newConfigsString, tryUpdate, next) {
                if (tryUpdate) {
                    ...
                    return connection.query("UPDATE configuration SET value = ? WHERE `key` = ?", [newConfigsString, "mailingList"], function (err) {
